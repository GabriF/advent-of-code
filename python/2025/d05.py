...
def _range_overlap_helper(r_1: tuple[int, int], r_2: tuple[int, int]):
...
def build_ranges(l: list[str]) -> list[tuple[int, int]]:
    ranges_list = []
    for record in l:
        tokens = record.split("-")
        v_1 = int(tokens[0])
        v_2 = int(tokens[1])
        add_range(ranges_list, (v_1, v_2))
    return ranges_list


def add_range(ranges_list: list[tuple[int, int]], new_range: tuple[int, int]):
    for i, curr_range in enumerate(ranges_list):
        if range_overlap(new_range, curr_range):
            new_range = (min(new_range[0], curr_range[0]), max(
                new_range[1], curr_range[1]))
            ranges_list.pop(i)
