...
def partition(string: str, part_len: int) -> list[str]:
    ...
    string_list = []
    for start in range(0, len(string), part_len):
        string_list.append(string[start:start+part_len])
    return string_list
...
def count_invalid(number_range: list[str]) -> int:
    ...
    s_2 = sum(n for n in range(n_1, n_2 + 1)
              if is_invalid(n, get_factors(len(str(n)))))
    return (s_1, s_2)
